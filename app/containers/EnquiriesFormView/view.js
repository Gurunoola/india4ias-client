  ...
  return (
    <div className=''>
          ...
          <div className='col-md-2'>
            <svg width="100" height="100" >
              ...
              <text x="50%" y="50%" alignment-baseline="central" text-anchor="middle" font-family="sans-serif" font-size="30" fill="#fff">
                {`${formData.name &&  _.upperCase(getInitials(formData.name))}`}
              </text>
            </svg>
          </div>
          <div className='col-md-5'>
            <h3 className='pl-3 text-primary text-capitalize mt-2'>{_.capitalize(formData.name)}</h3>
      ...
      <div className='col-md-12 p-0'>
        <Collapsiable disable={true} title={`${_.capitalize(title)} Details`} body={<HtmlTableBuilder json={_.omit({ ...formData }, ['name', 'phoneNumber', 'id', 'rescheduledDate', 'status', 'gender'])} cols={3} mode={'view'} />} defaultCollapse={true} />
      </div>
    </div>
  );
}