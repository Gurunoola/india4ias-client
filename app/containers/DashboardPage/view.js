    ...
    <div className=''>
      ...
      <div className='col-md-12 p-4 bg-white text-center text-md-left text-lg-left '>
        <div className='row m-0 p-0'>
          <div className='col-md-2'>
            <svg width="100" height="100" >
              ...
              <text x="50%" y="50%" alignment-baseline="central" text-anchor="middle" font-family="sans-serif" font-size="30" fill="#fff">
                {`${formData.firstName && upperCase(formData.firstName[0])}${formData.lastName && upperCase(formData.lastName[0])}`}
              </text>
            </svg>
          </div>
          <div className='col-md-5'>
            <h3 className='pl-3 text-primary text-capitalize mt-2'>{capitalize(formData.firstName)} {capitalize(formData.lastName)}</h3>
            <div className='row m-0 p-0'>
          ...
          <div className='col-md-5'>
            <div className='col p-1'><Icon icon={'easel'} size='14px' type={'primary'}/> {upperCase(formData && formData.class && formData.class.name + formData.class.section)}</div>
            ...
            <div className='col p-1'><Icon icon={'gender-ambiguous'} size='14px' type={'primary'}/> {capitalize(formData.gender)}</div>
          </div>
        </div>
      </div>
      <div className='col-md-12 p-0'>
        <Collapsiable title={`${capitalize(title)} Details`} body={<HtmlTableBuilder json={omit({ ...formData }, ['firstName', 'lastName', 'primaryMobile', 'id', 'middleName', 'parents', 'addresses', 'gender', 'dob'])} cols={3} mode={'view'} />} defaultCollapse={true} />
      </div>
      <div className='col p-0 '>
        ...
        <Collapsiable title={'Parent Details'} body={
          <div>
             {/* <p className='p-3 bg-custom-gray'><strong>{chain(formData).get('parents[0].relationship').capitalize().value()}</strong></p> */}
            {
              formData.parents && formData.parents.length > 0 && <HtmlTableBuilder json={omit({ ...formData.parents[0] }, ['id'])} cols={3} mode={'view'} />
            }
            {/* <p className='p-3 bg-custom-gray'><strong>{chain(formData).get('parents[1].relationship').capitalize().value()}</strong></p> */}
            {
              formData.parents && formData.parents.length > 0 && <HtmlTableBuilder json={omit({ ...formData.parents[1] }, ['id'])} cols={3} mode={'view'} />
            }
